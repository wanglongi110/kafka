...
object Defaults {
  ...
  // HOT FIX BATCH THE CONTROLLED SHUTDOWN
...
object KafkaConfig {
  ...
  // HOT FIX BATCH THE CONTROLLED SHUTDOWN
  val ControlledShutdownPartitionBatchSizeDoc = "The number of partitions for state change in one batch during controlled shutdown."
  ...
  private val configDef = {
    ...
    new ConfigDef()
      ...
      // HOT FIX BATCH THE CONTROLLED SHUTDOWN
      .define(ControlledShutdownPartitionBatchSizeProp, INT, Defaults.ControlledShutdownPartitionBatchSize, atLeast(1), MEDIUM, ControlledShutdownPartitionBatchSizeDoc)

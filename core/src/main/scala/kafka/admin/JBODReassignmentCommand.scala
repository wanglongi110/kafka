...
object JBODReassignmentCommand {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      if(opts.options.has(opts.skipUrpOpt))
        ...
        executeReassignment(zkUtils, opts)
    } catch {
      case e: Throwable =>
  ...
  def executeReassignment(zkUtils: ZkUtils, opts: JBODReassignmentCommandOptions): Unit = {
    ...
    newPartitionAssignment.toSeq.grouped(batchSize).foreach { assignment =>
      ...
      ReassignPartitionsCommand.executeAssignment(zkUtils, None, formatAsJson(assignment), ReassignPartitionsCommand.Throttle(-1))
    }
  }

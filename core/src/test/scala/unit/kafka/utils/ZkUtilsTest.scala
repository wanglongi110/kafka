...
class ZkUtilsTest extends ZooKeeperTestHarness {
  ...
  def testCheckAndDeleteMultiOps() {
    ...
    try {
      zkUtils.transactionalDeletePath(checkVersion + 1, deletePath)
    } catch {
      case _: ZkBadVersionException => fail = true
    }
    assertTrue(fail)
  ...
  def testCheckAndDeleteRecursiveMultiOps() {
    ...
    try {
      zkUtils.transactionalDeletePathRecursive(checkVersion+1, deletePaths.head)
    } catch {
      case _: ZkBadVersionException => fail = true
    }
    assertTrue(fail)
  ...
  def testCheckAndUpdateMultiOps() {
    ...
    try {
      zkUtils.transactionalUpdatePersistentPath(checkVersion+1, updatePath, "somedata-2")
    } catch {
      case _: ZkBadVersionException => fail = true
    }
    assertTrue(fail)
    ...
    zkUtils.updatePersistentPath(ZkUtils.ControllerEpochPath, "newdata")
    zkUtils.transactionalUpdatePersistentPath(checkVersion+1, updatePath, "somedata-3")

    assertEquals("somedata-3", zkUtils.readData(updatePath)._1)
  }
  ...
  def testCheckAndCreateMultiOps() {
    ...
    try {
      zkUtils.transactionalCreatePersistentPath(checkVersion+1, createPath, "somedata")
    } catch {
      case _: ZkBadVersionException => fail = true
    }
    assertTrue(fail)
